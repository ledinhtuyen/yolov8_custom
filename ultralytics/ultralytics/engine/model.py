...
class Model(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        if Path(model).suffix in {".yaml", ".yml"}:
            self._new(model, task=task, verbose=verbose, *args, **kwargs)
        else:
            self._load(model, task=task, *args, **kwargs)
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False, *args, **kwargs) -> None:
        """
    ...
    def _load(self, weights: str, task=None, *args, **kwargs) -> None:
        """
        ...
        if Path(weights).suffix == ".pt":
            self.model, self.ckpt = attempt_load_one_weight(weights, *args, **kwargs)
        ...
        **kwargs,
        ...
        if not args.get("resume"):  # manually set model only if not resuming
            if hasattr(self, "branch"):
                self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml, branch=self.branch, task=self.task)
            else:
